<template>
    <div>
        ...
        <space-between>
            <div class="search">
                <a-input-search placeholder="请输入搜索内容" enter-button v-model:value="keyword" @search="onSearch">
                </a-input-search>
                <a-button @click="backAll" v-if="isSearch == true" id="btn">返回</a-button>
            </div>
        ...
        <a-table :dataSource="list" :columns="columns" :pagination="false">
            ...
            <template #count="date">
                <a href="javascript:;" @click="upDateCount('IN_COUNT',date.record)">入库</a>
                {{ date.record.count }}
                <a href="javascript:;" @click="upDateCount('OUT_COUNT',date.record)">出库</a>
            </template>
